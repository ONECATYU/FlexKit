...
- (instancetype)initWithItem:(WSFlexLayoutItem *)item
{
  if (self = [super init]) {
    _item = item;
...
- (BOOL)isLeaf
{
  ...
  if (self.isEnabled) {
    for (WSFlexLayoutItem *subview in self.item.children) {
...
- (void)applyLayout
{
  [self calculateLayoutWithSize:self.item.flexFrame.size];
  YGApplyLayoutToViewHierarchy(self.item, NO);
}

- (void)applyLayoutPreservingOrigin:(BOOL)preserveOrigin
{
  [self calculateLayoutWithSize:self.item.flexFrame.size];
  YGApplyLayoutToViewHierarchy(self.item, preserveOrigin);
}

- (void)applyLayoutPreservingOrigin:(BOOL)preserveOrigin dimensionFlexibility:(YGDimensionFlexibility)dimensionFlexibility
{
  CGSize size = self.item.flexFrame.size;
...
- (CGSize)calculateLayoutWithSize:(CGSize)size
{
  ...
  YGAttachNodesFromViewHierachy(self.item);
  ...
  YGMeasureMode heightMode)
{
    ...
    WSFlexLayoutItem *item = (__bridge WSFlexLayoutItem*) YGNodeGetContext(node);
    CGSize sizeThatFits = CGSizeZero;
    
    sizeThatFits = [item measureItemWithSize:(CGSize){
...
static BOOL YGNodeHasExactSameChildren(const YGNodeRef node, NSArray<WSFlexLayoutItem *> *subviews)
{
...
static void YGAttachNodesFromViewHierachy(WSFlexLayoutItem *const view)
{
  ...
  if (yoga.isLeaf) {
    ...
    NSMutableArray<WSFlexLayoutItem *> *subviewsToInclude = [[NSMutableArray alloc] initWithCapacity:view.children.count];
    for (WSFlexLayoutItem *subview in view.children) {
    ...
    for (WSFlexLayoutItem *const subview in subviewsToInclude) {
...
static void YGApplyLayoutToViewHierarchy(WSFlexLayoutItem *view, BOOL preserveOrigin)
{
  ...
  const CGPoint origin = preserveOrigin ? view.flexFrame.origin : CGPointZero;
  view.flexFrame = (CGRect) {
    .origin = {
...
- (void)markChildrenDirty {
    for (NSUInteger i = 0; i < self.item.children.count; i++) {
        WSFlexLayoutItem *child = self.item.children[i];
        if (child.children.count > 0) {
            [child.yoga markChildrenDirty];
            continue;
        }
        if (![child isKindOfClass:UIView.class] &&
            ![child isKindOfClass:CALayer.class]) {
            continue;
        }
...
- (void)adjustsViewHierarchy {
    YGAdjustsViewHierarchy(self.item, nil, nil);
}

static void YGAdjustsViewHierarchy(WSFlexLayoutItem *currentItem, UIView *superView,CALayer *superLayer)
{
    NSCAssert([NSThread isMainThread], @"Adjusts view hierarchy should only be done on the main thread.");
    
    if (!currentItem.yoga.isIncludedInLayout) {
        return;
    }
    
    if ([currentItem isKindOfClass:UIView.class]) {
        UIView *view = (UIView *)currentItem;
        UIView *currentSuperView = superView;
        if ([currentItem.parent isKindOfClass:UIView.class]) {
            currentSuperView = (UIView *)currentItem.parent;
        }
        if (currentSuperView) {
            [view removeFromSuperview];
            [currentSuperView addSubview:view];
        }
        superView = view;
        superLayer = superView.layer;
    }
    if ([currentItem isKindOfClass:CALayer.class]) {
        CALayer *layer = (CALayer *)currentItem;
        CALayer *currentSuperLayer = (CALayer *)superLayer;
        if ([currentItem.parent isKindOfClass:CALayer.class]) {
            currentSuperLayer = (CALayer *)currentItem.parent;
        } else if ([currentItem.parent isKindOfClass:UIView.class]) {
            currentSuperLayer = [(UIView *)currentItem.parent layer];
        }
        if (currentSuperLayer) {
            [layer removeFromSuperlayer];
            [currentSuperLayer addSublayer:layer];
        }
        superLayer = layer;
    }
    
    if (!currentItem.yoga.isLeaf) {
        for (NSInteger i = 0; i < currentItem.children.count; i++) {
            WSFlexLayoutItem *childItem = currentItem.children[i];
            YGAdjustsViewHierarchy(childItem, superView, superLayer);
        }
    }
}
