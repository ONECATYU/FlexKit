...
- (instancetype)initWithVirtualView:(YGLayoutDiv *)view
{
    if (self = [super init]) {
        _view = view;
...
{
    if (self.isEnabled) {
        for (YGLayoutDiv *subview in self.view.children) {
...
static BOOL YGNodeHasExactSameChildren(const YGNodeRef node, NSArray<YGLayoutDiv *> *subviews)
{
...
static void YGAttachNodesFromViewHierachy(YGLayoutDiv *const view)
{
    ...
    if (yoga.isLeaf) {
        ...
        NSMutableArray<YGLayoutDiv *> *subviewsToInclude = [[NSMutableArray alloc] initWithCapacity:view.children.count];
        for (YGLayoutDiv *subview in view.children) {
        ...
        for (YGLayoutDiv *const subview in subviewsToInclude) {
...
static void YGApplyLayoutToViewHierarchy(YGLayoutDiv *div, BOOL preserveOrigin)
{
    const YGLayout *yoga = div.yoga;
